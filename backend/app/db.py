...
CREATE TABLE IF NOT EXISTS script_versions(    -- 脚本版本（多版本共存；script_case 始终=激活版本的映射）
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  script_id INTEGER, version TEXT,
  framework TEXT,                              -- 各版本可独立识别框架（激活时回写 scripts）
  file_name TEXT, file_path TEXT,              -- 每版本独立原件，可单独下载/覆盖
  cases_json TEXT,                             -- 该版本映射快照 [[case_id,module,priority],...]
  created_at TEXT
);
...
# udid 即 adb serial：ip:port 的进入该品牌真机执行池；非网络格式的为演示占位（探活后会显示离线）
SEED_DEVICES = [
    ("奥迪 MMI 台架 #1", "奥迪", "192.168.2.32:5555", "1920×816", "Android 14 (AAOS)", 1, 1),
    ("奥迪 MMI 台架 #2", "奥迪", "192.168.2.89:5555", "1920×816", "Android 14 (AAOS)", 1, 1),
    ("保时捷台架（演示）", "保时捷", "PORSCHE-CAYENNE-002", "2560×980", "Android 11", 1, 1),
    ("大众台架（演示）", "大众", "VW-ID6-IVI-003", "1440×540", "Android 10", 0, 0),
]
...
def _migrate(conn: sqlite3.Connection):
    ...
    add = {
        ...
        "scripts": [("framework", "TEXT"), ("active_version_id", "INTEGER")],
        "devices": [("meta", "TEXT")],   # 探活采集的富信息 JSON：型号/API/locale/App版本
    }
    ...
    # 版本化回填（幂等）：多版本管理上线前的旧脚本各补一条「当前版本」记录。
    # 仓库脚本（version='仓库'）由 scanner 全权管理、无版本概念，跳过。
    have = {r["script_id"] for r in conn.execute("SELECT DISTINCT script_id FROM script_versions")}
    for s in conn.execute("SELECT * FROM scripts WHERE version != '仓库'").fetchall():
        if s["id"] in have:
            continue
        cases = [[r["case_id"], r["module"], r["priority"]] for r in conn.execute(
            "SELECT case_id, module, priority FROM script_case WHERE script_id = ? ORDER BY rowid",
            (s["id"],))]
        # 旧数据的版本列可能是「12个测试文件」这类文案 → 归一为 v1.0
        ver = s["version"] if (s["version"] or "").startswith("v") else "v1.0"
        vid = conn.execute(
            "INSERT INTO script_versions(script_id,version,framework,file_name,file_path,cases_json,created_at)"
            " VALUES(?,?,?,?,?,?,?)",
            (s["id"], ver, s["framework"], s["file_name"], s["file_path"],
             json.dumps(cases, ensure_ascii=False), s["created_at"])).lastrowid
        conn.execute("UPDATE scripts SET version = ?, active_version_id = ? WHERE id = ?",
                     (ver, vid, s["id"]))

    # 真台架入库（幂等）：REAL_DEVICE_MAP 硬编码废弃 → devices 表为执行设备的单一事实源。
    # 旧库的「奥迪台架 #1」占位 udid 改写为真 .32；.89 已恢复可用（2026-07-13 实测）补一行。
    udids = {r["udid"] for r in conn.execute("SELECT udid FROM devices")}
    if udids and "192.168.2.32:5555" not in udids:
        row = conn.execute("SELECT id FROM devices WHERE brand = '奥迪' AND udid NOT LIKE '%:%'"
                           " ORDER BY id LIMIT 1").fetchone()
        if row:
            conn.execute("UPDATE devices SET name='奥迪 MMI 台架 #1', udid='192.168.2.32:5555',"
                         " resolution='1920×816', os='Android 14 (AAOS)' WHERE id=?", (row["id"],))
        else:
            conn.execute("INSERT INTO devices(name,brand,udid,resolution,os,online,agent_ready)"
                         " VALUES('奥迪 MMI 台架 #1','奥迪','192.168.2.32:5555','1920×816',"
                         "'Android 14 (AAOS)',1,1)")
    if udids and "192.168.2.89:5555" not in udids:
        conn.execute("INSERT INTO devices(name,brand,udid,resolution,os,online,agent_ready)"
                     " VALUES('奥迪 MMI 台架 #2','奥迪','192.168.2.89:5555','1920×816',"
                     "'Android 14 (AAOS)',1,1)")

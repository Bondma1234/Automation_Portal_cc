...
export async function render() {
  ...
  // 默认只勾选第一个品牌（奥迪 = 当前唯一真机台架），其余手动勾选
  const brandChecks = brands.map((b, i) => `<label style="font-size:13px;color:var(--color-text-primary);display:inline-flex;align-items:center;gap:4px;"><input type="checkbox" class="exb" value="${b}"${i === 0 ? ' checked' : ''}> ${b}</label>`).join('');
